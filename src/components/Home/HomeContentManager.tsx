    ...
    try {
      if (editingItem) {
        await supabase
          ...
          .update(servicePreviewForm)
          .eq('id', editingItem.id)
      } else {
        await supabase
          .from('home_services_preview')
          .insert([servicePreviewForm])
      }
      await fetchAllData()
      setShowModal(false)
      resetForms()
              ...
              {modalType === 'service' && (
                <form onSubmit={handleServicePreviewSubmit} className="space-y-4">
                  ...
                  <div>
                    ...
                    <input
                      type="url"
                      value={servicePreviewForm.image_url}
                      onChange={(e) => setServicePreviewForm({ ...servicePreviewForm, image_url: e.target.value })}
                      className="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                    />
                  </div>
                  <div>
